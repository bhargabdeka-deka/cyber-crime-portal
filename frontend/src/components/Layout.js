      ...
      {!isMobile && (
        <aside className={`bg-white border-r border-slate-200 flex flex-col shrink-0 transition-all duration-300 ${collapsed ? "w-16" : "w-56"}`}>
          ...
          <nav className="flex-grow py-4 px-2 space-y-1">
            {NAV.map(item => {
              const active = location.pathname === item.path ||
                             (item.path === "/admin/dashboard" && location.pathname === "/dashboard");
              return (
                <button
                  key={item.path}
          ...
          <button
            ...
            className="flex items-center justify-center py-2 border-t border-slate-100 text-slate-400 hover:text-slate-600 transition-colors text-xs"
          >
        ...
        <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-5 shrink-0">
          ...
          <div className="flex items-center gap-3">
            <div className="text-xs font-medium text-slate-500 hidden sm:block">
              {user?.name}
              <span className="ml-1 px-1.5 py-0.5 bg-slate-100 rounded text-[10px] text-slate-500 capitalize">{user?.role}</span>
            </div>
            {isMobile && (
              <button
